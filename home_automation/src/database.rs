...
use crate::models::{ReadDevice, User, NewUser, NewDevice}; // estruturas de dados necessárias
...
pub fn get_device_by_id(conn: &PgConnection, other_device_id: &i32) -> QueryResult<ReadDevice> {
    use crate::schema::devices::dsl::*;
    devices.filter(id.eq(other_device_id)).first::<ReadDevice>(conn)
}

// Cria um novo registro (dispositivo) na tabela "devices", definida em "schema.rs"
pub fn create_device(conn: &PgConnection, new_device: &NewDevice) -> QueryResult<usize> {
...
pub fn update_device(conn: &PgConnection, other_device_id: &i32, new_status: &str) -> QueryResult<usize> {
    use crate::schema::devices::dsl::*;
    diesel::update(devices.filter(id.eq(other_device_id)))
        .set(status.eq(new_status))
        .execute(conn)
}

// Deleta um dispositivo com base no id
pub fn delete_device(conn: &PgConnection, other_device_id: &i32) -> QueryResult<usize> {
    use crate::schema::devices::dsl::*;
    diesel::delete(devices.filter(id.eq(other_device_id)))
        .execute(conn)
}

// Insere um usuário 
pub fn create_user(conn: &PgConnection, new_user: NewUser) -> QueryResult<usize> {
...
pub fn get_user_by_email(conn: &PgConnection, other_email: &str) -> QueryResult<User> {
    use crate::schema::users::dsl::*;
    users.filter(email.eq(other_email)).first::<User>(conn)
}
